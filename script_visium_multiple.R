...
for(i in 1:length(path_list_to_file_patient)){
  ...
  for(j in 1:length(path_list_to_file_patient[i])){
    ...
    spatial_features_plot <- SpatialPlot(patient_merge_data,
                                         features = c("CD8A","CD8B","CD4", "CTLA4", "CD68","CD163",
                                                      "MRC1","FCGR3B", "S100A8", "S100A9", "KLRD1", "NKG7"),
                                         alpha = c(0.05, 1),
                                         min.cutoff = 0)
    
    pdf("spatial_features_plot_genes_for_agnes.pdf",
        height = 8*length(c("CD8A","CD8B","CD4", "CTLA4", "CD68","CD163",
                   "MRC1","FCGR3B", "S100A8", "S100A9", "KLRD1", "NKG7"))/2,
        width = 16)
    plot(spatial_features_plot)
    dev.off()
    # 
    ...
    ES_full <- enrichIt(obj = as.matrix(patient_merge_data@assays[["SCT"]]@counts),
    ...
    ESmakfull <- enrichIt(obj = as.matrix(patient_merge_data@assays[["SCT"]]@counts),

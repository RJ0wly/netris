convert_gene_id_to_ensembl_from_symbol <- function(data,gene_list){
  logical_biological_id_ENSEMBL <- FALSE
  logical_biological_id_SYMBOL <- TRUE
  is_in_gene_list <- FALSE
  # check type of entry of genes
  if(grepl("ENSG",colnames(data)[1])){
    logical_biological_id_ENSEMBL <- TRUE
    logical_biological_id_SYMBOL <- FALSE
    logical_convert_id_ENSEMBL <- FALSE
    # check the entry of ENSEMBL ID
    if(logical_biological_id_ENSEMBL){
      if(grep(".",colnames(data)[1])){
        logical_convert_id_ENSEMBL <- TRUE
      }
    }
    # change ENSEMBL ID format to get rid of "." in it
    if(logical_convert_id_ENSEMBL){
      ensg_point_look_up <- gsub("\\.[0-9]*$", "", colnames(data))
    }
    # converting ENSEMBL ID into SYMBOL return a dataframe containing SYMBOL, 
    # ENTREZID and ENSEMBL
    if(logical_biological_id_ENSEMBL){
    ...
    # renaming column with ENSEMBL ID without "." in it
    colnames(data) <- ensg_point_look_up
    # select only ENSEMBL ID that were convert
    data <- data[,convert_ENSEMBL_id$ENSEMBL]
    # renaming column with SYMBOL
    colnames(data) <- convert_ENSEMBL_id$SYMBOL
    cat("converting ENSEMBL ID to SYMBOl, done\n")
    
  }else{
    cat("genes IDs is already in the good format\n")
  }
  return(data)
}

case_when_character_type <-function(var_of_interest,
                               min=0,
                               max=1,
                               status=c("neg","pos","int")){
  if(length(status) > 3){
    stop("status: length vector is not equal to 3 (superior)")
  }else if(length(status) < 3){
    stop("status: length vector is not equal to 3 (inferior)")
  }else{
    case_when(var_of_interest < min ~ status[1],
              var_of_interest > max ~ status[2],
              var_of_interest > min & var_of_interest < max ~ status[3])
  }
}

TCGA_DEG_analysis <- function(name_cancer_vec){
  ...
  file_path <- paste0(paste0(paste0("/home/rj/Desktop/TCGA_project/Project_TCGA_signature/datasets/",
                                     "TCGA_"),
                             as.character(name_cancer_vec)),
                                      ".RDS")
  cat(paste0(as.character(name_cancer_vec),"\n"))
  ...
  df_meta_data <- data %>% dplyr::select(intersect(score_NTN1$V1,genes_name_list$V1))
  
  
  compute_zscore <- sapply(df_meta_data, function(df) (df-mean(df))/sd(df))
  
  mean_zscore_by_patient <- apply(compute_zscore,1,mean)
  
  df_meta_data <- cbind(df_meta_data,"zscore"=mean_zscore_by_patient)
  ...
  fit <- survminer::surv_fit(Surv(OS.time,OS) ~ + netrin_status, data = df_surv)
  ...
  fit2 <- survminer::surv_fit(Surv(OS.time,OS) ~ + netrin_status, data = df_surv2)
  ...
  survival_plot <- arrange_ggsurvplots(splots,
                                       print = FALSE,
                                       ncol = 2, 
                                       nrow = 1,
                                       isk.table.height = 0.4)
  
  obj <- list("plot"=main_p,"result"=keep,"survplot"=survival_plot)
  return(obj)
}
